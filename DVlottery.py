...
# ---------------------- MOBILE CAMERA CSS OVERLAY ----------------------
def inject_camera_css():
    """Inject CSS to add DV guidelines overlay to mobile camera"""
    st.markdown("""
    <style>
    /* Add green overlay to camera preview */
    ...
    /* Add guideline text above camera */
...
# ---------------------- VISUAL GUIDE IMAGE ----------------------
def create_camera_guide_image():
    """Create a visual guide showing how to position in camera"""
    ...
    oval_width, oval_height = 180, 240
    draw.ellipse([
    ...
    # Draw eye level line
    eye_y = center_y - 40
    draw.line([(50, eye_y), (250, eye_y)], fill='green', width=2)
    
    # Add labels
    draw.text((120, center_y - oval_height//2 - 15), "HEAD TOP", fill='blue')
    draw.text((135, center_y + oval_height//2 - 10), "CHIN", fill='purple')
    draw.text((125, eye_y - 15), "EYE LEVEL", fill='green')
    
    # Add mobile instructions
    draw.text((20, 320), "📱 Position face in green oval", fill='black')
    draw.text((20, 340), "👁️ Align eyes with green line", fill='black')
    draw.text((20, 360), "📸 Take photo when centered", fill='black')
...
with st.sidebar:
    ...
    # Show visual guide in sidebar
    guide_img = create_camera_guide_image()
    st.image(guide_img, caption="📱 Camera Positioning Guide", use_column_width=True)

# Main content with mobile camera instructions
st.markdown("""
<div class="camera-guide-overlay">
📱 <strong>MOBILE CAMERA GUIDELINES</strong><br>
When you click "Browse files" and select "Take Photo", your camera will open with GREEN GUIDELINES to help position your face perfectly!
</div>

<div class="guide-points">
<div>🎯 Center Face in Frame</div>
<div>👁️ Eyes at Middle Height</div> 
<div>📏 Head Fills 50-69%</div>
</div>
""", unsafe_allow_html=True)

# File uploader with camera option
uploaded_file = st.file_uploader(
    "📤 Upload Your Photo or Take with Camera", 
    type=["jpg", "jpeg", "png"],
    help="Click 'Browse files' → Select 'Take Photo' to use camera with DV guidelines"
)
...
if uploaded_file:
    ...
    ### 🚀 How it works:
    1. **Upload** your photo or **Take Photo** with camera
    ...
    - **Green guidelines** appear in camera view
    - **Perfect positioning** assistance
...
# Footer
st.markdown("---")
st.markdown("*DV Lottery Photo Editor | Mobile-friendly with camera guidelines*")
